...
class Pythia8(MCRun):
    ...
    def _cross_section(self, kin):
        ...
        if (kin.p1.A or 0) > 1 or (kin.p2.A or 1) > 1:
            ...
            total=info.sigmaGen(0),
            inelastic=info.sigmaGen(0) - info.sigmaGen(102),
            elastic=info.sigmaGen(102),
            diffractive_ax=info.sigmaGen(104),
            diffractive_xb=info.sigmaGen(103),
            diffractive_xx=info.sigmaGen(105),
            diffractive_axb=info.sigmaGen(106)  
        # standard Pythia (pp)  
        else: 
